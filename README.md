- [lichess-bot-gui，版本1.4](#lichess-bot-gui版本14)
- [开源协议合规说明](#开源协议合规说明)
- [项目主要功能说明](#项目主要功能说明)
- [使用教程](#使用教程)
  - [一、前置硬性要求（必看）](#一前置硬性要求必看)
  - [二、注册并升级Bot账号](#二注册并升级bot账号)
    - [2.1 新建账号](#21-新建账号)
    - [2.2 生成API Token（关键）](#22-生成api-token关键)
    - [2.3 将账号升级为Bot身份（二选一）](#23-将账号升级为bot身份二选一)
  - [三、安装Python依赖](#三安装python依赖)
  - [四、下载国际象棋引擎 Stockfish](#四下载国际象棋引擎-stockfish)
  - [五、配置 config.yml（核心配置）](#五配置-configyml核心配置)
    - [① 填入API令牌](#-填入api令牌)
    - [② 引擎路径（engine）](#-引擎路径engine)
    - [③ 引擎强度（uci\_options，自由调节）](#-引擎强度uci_options自由调节)
    - [④ 对局过滤（接受/拒绝挑战）](#-对局过滤接受拒绝挑战)
    - [⑤ 其他常用配置](#-其他常用配置)
  - [六、启动机器人](#六启动机器人)
    - [启动成功标志](#启动成功标志)
    - [停止机器人](#停止机器人)
  ...
  - [八、进阶优化](#八进阶优化)
  - [九、重要规则提醒](#九重要规则提醒)
- [修改配置相关其他内容](#修改配置相关其他内容)
  - [`修改机器人配置`组件，用来修改config.yml](#修改机器人配置组件用来修改configyml)
  - [`修改gui配置`组件，用来修改gui\_config.json](#修改gui配置组件用来修改gui_configjson)
  - [`修改gui样式`组件，用来修改style.txt](#修改gui样式组件用来修改styletxt)
- [实用组件](#实用组件)
- [gui\_config.json配置修改说明](#gui_configjson配置修改说明)
  - [chess.svg.board()尺寸与样式](#chesssvgboard尺寸与样式)
  - [game\_viewer要观看的账号](#game_viewer要观看的账号)
