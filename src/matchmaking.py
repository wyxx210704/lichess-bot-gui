...
import contextlib
import model
from timer_ import Timer, days, seconds, minutes, years
from collections import defaultdict
from collections.abc import Sequence
from lichess import Lichess, RateLimitedError
from config import Configuration
from typing import cast, TypeAlias
from blocklist import OnlineBlocklist
from lichess_types import UserProfileType, PerfType, EventType, FilterType, ChallengeType
...
class Matchmaking:
    ...
    def should_create_challenge(self) -> bool:
        ...
        if challenge_expired:
            ...
            logger.info(f"挑战编号{self.challenge_id}已经取消")
    ...
    def choose_opponent(self) -> tuple[str | None, int, int, int, str, str]:
        ...
        logger.info(f"使用{override_choice or '默认'}的匹配配置。")
        send_text(f"使用{override_choice or '默认'}的匹配配置。")
        ...
        logger.info(f"正在寻找对手评级在[{min_rating}, {max_rating}]范围内的 {game_type}游戏...")
        send_text(f"正在寻找对手评级在[{min_rating}, {max_rating}]范围内的 {game_type}游戏...")
        ...
        logger.info(f"找到{len(online_bots)}个在线机器人")
        send_text(f"找到{len(online_bots)}个在线机器人")
        online_bots = list(filter(is_suitable_opponent, online_bots))
        logger.info(f"从{len(online_bots)}个合适的对手中进行选择")
        send_text(f"从{len(online_bots)}个合适的对手中进行选择")
    ...
    def challenge(self, active_games: set[str], challenge_queue: MULTIPROCESSING_LIST_TYPE, max_games: int) -> None:
        ...
        logger.info("挑战随机一个机器人")
        send_text("挑战随机一个机器人")
        self.update_user_profile()
        bot_username, base_time, increment, days, variant, mode = self.choose_opponent()
        logger.info(f"将向 {bot_username} 挑战一场 {variant} 游戏。")
        send_text(f"将向 {bot_username} 挑战一场 {variant} 游戏。")
        challenge_id = self.create_challenge(bot_username, base_time, increment, days, variant, mode) if bot_username else ""
        logger.info(f"挑战编号是{challenge_id or 'None'}.")
        send_text(f"挑战编号是{challenge_id or 'None'}.")
    ...
    def show_earliest_challenge_time(self) -> None:
        ...
        if self.matchmaking_cfg.allow_matchmaking:
            ...
            logger.info(f"接下来的挑战将在{earliest_challenge_time.strftime('%c')}之后出现。")
            send_text(f"接下来的挑战将在{earliest_challenge_time.strftime('%c')}之后出现。")
    ...
    def declined_challenge(self, event: EventType) -> None:
        ...
        logger.info(f"{opponent}拒绝了{challenge}: {reason}")
        send_text(f"{opponent}拒绝了{challenge}: {reason}")
