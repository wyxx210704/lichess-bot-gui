...
import model, lichess
from config import Configuration, change_value_to_list
from timer_ import Timer, msec, seconds, msec_str, sec_str, to_seconds
from lichess_types import (ReadableType, ChessDBMoveType, LichessEGTBMoveType, OPTIONS_GO_EGTB_TYPE, OPTIONS_TYPE,
...
class EngineWrapper:
    ...
    def print_stats(self) -> None:
        ...
        for line in self.get_stats():
            logger.info(line)
        send_text('对局进行中，对局信息请跳转到下一个标签页查看')
...
def first_move_time(game: model.Game) -> chess.engine.Limit:
    ...
    logger.info(f"为游戏 {game.id} 搜索了 {sec_str(search_time)} 秒的时间。")
    send_text(f"为游戏 {game.id} 搜索了 {sec_str(search_time)} 秒的时间。")
...
def game_clock_time(board: chess.Board,
    ...
    logger.info(f"正在查找游戏 {game.id} 的“白方时间”为 {msec_str(times['wtime'])} 毫秒，“黑方时间”为 {msec_str(times['btime'])} 毫秒。")
    send_text(f"正在查找游戏 {game.id} 的“白方时间”为 {msec_str(times['wtime'])} 毫秒，“黑方时间”为 {msec_str(times['btime'])} 毫秒。")
