...
import engine_wrapper, model, lichess, matchmaking
import json
import logging
...
import contextlib
from blocklist import OnlineBlocklist
from config import load_config, Configuration, log_config
from conversation import Conversation, ChatLine
from timer_ import Timer, seconds, msec, hours, to_seconds
...
def start(li: lichess.Lichess, user_profile: UserProfileType, config: Configuration, logging_level: int,
    ...
    """
    logger.info(f"您现已连接至 {config.url} 并正在等待挑战。")
    send_text(f"您现已连接至 {config.url} 并正在等待挑战。")
...
def log_proc_count(change: str, active_games: set[str]) -> None:
    ...
    logger.info(f"{symbol} 进程 {change}. 计数: {len(active_games)}. 编号: {active_games or None}")
    send_text(f"{symbol} 进程 {change}. 计数: {len(active_games)}. 编号: {active_games or None}")
    ...
    with multiprocessing.pool.Pool(max_games + 1) as pool:
        while not (stop.terminated or (one_game and one_game_completed) or stop.restart):
            ...
            if event["type"] == "local_game_done":
                ...
                log_proc_count("释放", active_games)
...
def accept_challenges(li: lichess.Lichess, challenge_queue: MULTIPROCESSING_LIST_TYPE, active_games: set[str],
    ...
    while len(active_games) < max_games and challenge_queue:
        ...
        try:
            logger.info(f"接受 {chlng}")
            li.accept_challenge(chlng.id)
            active_games.add(chlng.id)
            log_proc_count("排队等候", active_games)
...
def start_game_thread(active_games: set[str], game_id: str, play_game_args: PlayGameArgsType, pool: POOL_TYPE) -> None:
    ...
    log_proc_count("使用中", active_games)
...
def print_move_number(board: chess.Board) -> None:
    """Log the move number."""
    logger.info("")
    logger.info(f"步数: {len(board.move_stack) // 2 + 1}")
    send_text(f"步数: {len(board.move_stack) // 2 + 1}")
...
def tell_user_game_result(game: model.Game, board: chess.Board) -> None:
    ...
    if winner is not None:
        logger.info(f"{winning_name}胜利!")
        send_text(f"{winning_name}胜利!")
    ...
    simple_endings: dict[str, str] = {model.Termination.MATE: "棋局以将死胜利",
                                      model.Termination.RESIGN: f"{losing_name} 认输",
                                      model.Termination.ABORT: "棋局已中断"}

    if termination in simple_endings:
        logger.info(simple_endings[termination])
        send_text(simple_endings[termination])
    elif termination == model.Termination.DRAW:
        draw_results = [(board.is_fifty_moves(), "比赛因 50 次走棋未分胜负而判定为平局。"),
                        (board.is_repetition(), "比赛因三番重复局面而判定为平局。"),
                        (board.is_insufficient_material(), "比赛因资料不足而无法进行。"),
                        (board.is_stalemate(), "比赛以平局收场。"),
                        (True, "比赛双方协商决定以平局收场。")]
        messages = [draw_message for is_result, draw_message in draw_results if is_result]
        logger.info(messages[0])
        send_text(messages[0])
    elif termination == model.Termination.TIMEOUT:
        if winner:
            logger.info(f"{losing_name} forfeited on time.")
            send_text(f"{losing_name} forfeited on time.")
        else:
            ...
            send_text(f"{timeout_name} ran out of time, but {other_name} did not have enough material to mate.")
    elif termination:
        logger.info(f"棋局结束于{termination}")
        send_text(f"棋局结束于{termination}")
...
def intro() -> str:
    ...
    .   _/|
    .  // o\
    .  || ._)  lichess机器人 版本{__version__} 在{platform.system()} {platform.release()}
    .  //__\
    .  )___(   在 Lichess 上与机器人对弈
    """
...
def start_lichess_bot() -> None:
    ...
    logger.info(f"欢迎 {username}!")
    send_text(f"欢迎 {username}!")
...
def send_text(text:str):
    from thread_worker import get_worker
    wocker = get_worker()
    if wocker:
        wocker.send_text(text)
